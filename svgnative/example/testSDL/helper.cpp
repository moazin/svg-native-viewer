...
#if defined(USE_SKIA) && defined(TEST_SNV_SKIA)
#include <SkData.h>
#include <SkImage.h>
#include <SkStream.h>
#include <SkSurface.h>
...
#if defined(USE_CG) && defined(TEST_SNV_CG)
...
#if defined(USE_CAIRO) && defined(TEST_SNV_CAIRO)
...
void loadCurrentSVG(State *state)
{
    if (state->current_svg_document < state->filenames.size()){
        ...
        svg_file.open(state->filenames[state->current_svg_document]);
        if (!svg_file)
        {
            std::cout << "failed to load file " << state->filenames[state->current_svg_document] << std::endl;
            exit(0);
        }
        std::string svg_doc = "";
        std::string line;
...
#ifdef  defined(USE_CAIRO) && defined(TEST_SNV_CAIRO)
        {
...
{
    std::ifstream filename_file;
    filename_file.open("./../../tests/svg-filenames.txt");
    std::string filename;
    while(std::getline(filename_file, filename))
    {
        ...
        if (filename.find(",") != std::string::npos)
            continue;
        state->filenames.push_back("./../../tests/bound-tests-svgs/" + filename);
    }
    /*
       char filename[100];
...
void calculateBoundingBoxSNVCairo(State *state, SVGNative::Rect &bound, std::vector<SVGNative::Rect> &bounds)
{
    ...
    auto doc = std::unique_ptr<SVGNative::SVGDocument>(SVGNative::SVGDocument::CreateSVGDocument(copy_doc.c_str(), renderer));
    renderer->SetCairo(state->d_cairo_cr);
    doc->GetBoundingBox(bound);
    doc->GetSubBoundingBoxes(bounds);
    auto end = std::chrono::steady_clock::now();
