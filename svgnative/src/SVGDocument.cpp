...
std::int32_t SVGDocument::X() const
{
    if (!mDocument)
        return 0;

    return static_cast<std::int32_t>(mDocument->mViewBox[0]);
}

std::int32_t SVGDocument::Y() const
{
    if (!mDocument)
        return 0;

    return static_cast<std::int32_t>(mDocument->mViewBox[1]);
}
